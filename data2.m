function [obj_fun_matrix, c, B] = data2(cache,client,object,capacity,requested_object,demand,op,N,client_coor)
%{
fname= 'def.csv';
Data= csvread(fname);
cache=Data(1,1)
...
%}
 assgn_cost=[];
    [e f]=size(N);
    [g h]=size(client_coor);
 
    for i=1:f
      for j=1:2:h-1
        
        assgn_cost=[assgn_cost demand(1,ceil((j+1)/2))*sqrt((N(1,i)-client_coor(1,j)).^2 +(N(2,i)-client_coor(1,j+1)).^2)];
      end
    end
    op;
   
    assgn_cost;
  obj_fun_matrix=[op assgn_cost];
